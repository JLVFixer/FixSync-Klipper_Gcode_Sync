...
class PrinterStatusRow(QFrame):
    def __init__(self, name, host):
        ...
        self.link_btn.setIcon(icons.get_icon("external_link", "#B8B8B8", 13))
        self.link_btn.setFlat(True)
        self.link_btn.setFixedSize(24, 24)
        self.link_btn.setCursor(Qt.PointingHandCursor)
        self.link_btn.setToolTip(tr("sync.open_web_tooltip", host=host))
        self.link_btn.clicked.connect(self._open_web)
    ...
    def _open_web(self):
        QDesktopServices.openUrl(QUrl(f"http://{self.host}"))
...
class SyncPage(QWidget):
    ...
    def refresh_printers(self):
        ...
        for i, printer in enumerate(self.cfg["printers"]):
            ...
            row = PrinterStatusRow(printer["name"], printer["host"])
