...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.app_title_label.setStyleSheet("font-size: 15px; font-weight: 700;")
    ...
    def _load_footer_logo(self):
        """Escala el logo de Fixer Robotics para que entre en el sidebar
        sin recortarse, manteniendo su relación de aspecto original."""
        path = resource_path("assets/fixer.png")
        pixmap = QPixmap(path)
        if pixmap.isNull():
            self.footer_logo_label.setFixedSize(0, 0)
            return
        scaled = pixmap.scaled(
            FOOTER_LOGO_MAX_SIZE, FOOTER_LOGO_MAX_SIZE,
            Qt.KeepAspectRatio, Qt.SmoothTransformation,
        )
        self.footer_logo_label.setPixmap(scaled)
        self.footer_logo_label.setFixedSize(scaled.size())

    def retranslate_all(self):
        self.setWindowTitle(tr("app.window_title"))
        self.app_title_label.setText(tr("app.sidebar_name"))
    ...
    def _apply_theme(self):
        ...
        self.logo_label.setPixmap(icons.get_icon("printer", self.settings.get("accent_color", "#E53935"), 24).pixmap(24, 24))
